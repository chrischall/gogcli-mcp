...
6. Annotations: **every tool declares one — "unannotated" is not neutral.** `destructiveHint` defaults to TRUE in the spec, so leaving a create unannotated (which this list used to tell you to do) publishes it as destructive, and a client that must raise the same alarm for `gog_gmail_drafts_forward` as for `gog_gmail_send` has told the reader nothing. Pick one of three: `readOnlyHint: true` when the tool changes nothing; `destructiveHint: false` when it ONLY adds a new object and cannot overwrite or remove what exists (the `drafts_*` savers, `labels_create`); `destructiveHint: true` for everything else — deletes, overwrites, grades, run-escape-hatches, and anything whose effect reaches outside this account or persists as a standing rule (`filters_create`, `sendas_create`, `vacation_update`) even though it only "adds" a row. The error direction is deliberate: a wrong `false` costs safety, a wrong `true` costs a prompt, so anything ambiguous stays destructive. Check what you actually published by reading `tools/list` off the built server — the source is easy to mis-grep, and the annotation may come from a shared registrar rather than the call site.
