...
- **Exactly ONE copy of each MCP SDK lane** — runtime tools use the modular SDK v2 `@modelcontextprotocol/server`; the retained `@modelcontextprotocol/sdk` v1 package exists only for compatibility pieces inside `agents`. `packages/gogcli-mcp/tests/sdk-single-copy.test.ts` guards both lanes. The root `overrides` entry pins the legacy copy required by `agents`; do not point tool registrars back at it. Both `McpServer` classes carry private state, so duplicate copies produce nominal `TS2322` errors with no source-level API mismatch. Read the resolved paths in those errors before changing registrar signatures.
- **Exactly ONE `zod` in the tree, for the same reason** — `packages/gogcli-mcp/tests/zod-single-copy.test.ts` is the guard, and the root `overrides` carries a `zod` entry to hold it. Here the exact pin belongs to `@cloudflare/vitest-pool-workers` (root devDependency), so it takes the root slot that `@chrischall/mcp-utils` (peer `^4.4.0`) and the MCP SDK resolve zod from; workspaces asking for anything newer nest their own copy. `ZodType` carries brand-bearing internals, so TypeScript again compares the two **nominally**, and every schema handed to `registerTool` fails `TS2322: Type 'ZodString' is not assignable to type 'AnySchema'` — dependabot #333 produced **11,024** such errors from one patch-level bump, with nothing wrong in the source. Both overrides are dev-tree only and are never published. When bumping zod, bump the override with it.
