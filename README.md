...
- The escape hatches cannot skip a confirmation: every action a dedicated tool asks about (a Gmail send, a Chat post, a Drive share, a Classroom announcement or invitation, publishing a draft announcement or coursework, a Calendar create/update/respond) is refused by `gog_<service>_run` under each of gog's aliases for it, and by `gog_api_call` as the raw API method. `gog_<service>_run` also refuses changes it cannot preview (`drive bulk` permission rewrites, calendar move/delete/out-of-office/decline, classroom roster adds and returns), and every other `gog_api_call` write asks the user to confirm the exact method and payload first
